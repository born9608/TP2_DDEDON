...
grouped_data = total_df.groupby(['시', '계약년월'])[['면적당보증금']].mean().reset_index()
...
# 시단위와 계약년월에 따른 면적당매매가와 면적당보증금의 평균 계산
grouped_data_2 = total_df.groupby(['도', '계약년월'])[['면적당보증금']].mean().reset_index()
...
filtered_data_2['변화율'] = filtered_data_2.groupby('도')['면적당보증금'].pct_change() * 100
Do_rc = filtered_data_2[['도','계약년월','변화율','면적당보증금']]
filtered_data = filtered_data[['시'	,'계약년월'	,'면적당보증금'	,'도']]
...
merged_df.to_csv("최종시_보간_noise_2.csv")