...
grouped_data = total_df.groupby(['시군구', '계약년월'])[['면적당매매금']].mean().reset_index()
...
grouped_data_2 = city_apart[['시'	,'계약년월'	,'면적당매매금']]
grouped_data_2
...
filtered_data_2['변화율'] = filtered_data_2.groupby('시')['면적당매매금'].pct_change() * 100
# display(filtered_data_2)

Do_rc = filtered_data_2[['시','계약년월','변화율','면적당매매금']]
...
merged_df['면적당매매금'] = merged_df['면적당매매금'].interpolate(method='linear')

merged_df.info()

merged_df.info()

merged_df.to_csv("시군구단위_noise_보간데이터_2.csv")